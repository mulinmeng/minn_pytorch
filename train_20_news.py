...
if __name__ == "__main__":
    ...
    model_name = [MI_Net,MI_Net_RS,MI_Net_DS]
    dir_ = [os.path.join(path, file) for file in os.listdir(path)]
    # dir_ = ["/home/zhangwenqiang/PycharmProjects/minn/dataset/elephant_100x100_matlab.mat"]
    result_acc = np.zeros((len(model_name), len(dir_)))
    result_std = np.zeros((len(model_name), len(dir_)))
    for ca,_model in enumerate(model_name):
        print(_model.__name__, "\n", "-" * 40, "\n")
        for cb,dataset_name in enumerate(dir_):
            # data = load_data.load_dataset("elephant_100x100_matlab",n_folds)
            data = load_data.load_news(dataset_name, n_folds)
            ...
            for time in range(times):
                for ifold in range(n_folds):
                    ...
                    model.to(device)
                    optimizer = optim.SGD(model.parameters(), lr=5e-4, momentum=0.9,weight_decay=0.005)
                    print("epoch ", time, " folds ", ifold)
                    train_bags = data[ifold]['train']
                    test_bags = data[ifold]['test']
                    train_set = my_utils.convertToBatch(train_bags)
                    test_set = my_utils.convertToBatch(test_bags)
                    # print(train_set)
                    for epoch in range(_epochs):
                        train_loss, train_acc = train(model, train_set, optimizer,device)
